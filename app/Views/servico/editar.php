...
<div class="border mt-3">
    ...
    <form action="<?= url_to("servico.onSave") ?>" method="post">
        <input type="hidden" name="idOrcamento" value="<?=$servico->idOrcamento?>">
        ...
        <div class="m-3">
            ...
            <textarea class="form-control" name="descricao" id="" rows="10"><?= $servico->descricao ?></textarea>
        </div>
        <button class="btn btn-success m-3" type="submit"> <i class="bi bi-floppy"></i> Salvar</button>
        <a class="btn btn-secondary" href="<?= base_url("servico/listar/".$servico->idOrcamento)?>"> <i class="bi bi-list"></i> Serviços</a>
    </form>
</div>
<?= $this->endSection() ?>
<?= $this->section("script") ?>
